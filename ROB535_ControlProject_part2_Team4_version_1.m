...
accum_error = 0; % accumulated error
lane_side_prev = "center";
lane_side = lane_side_prev;
...
for i=1:loopsteps
    ...
    [testinput,accum_error, lane_side] = steering_obs(lpinp,TestTrack,Yprev,testinput,outputsteps,accum_error, Xobs, avoid_obs_flag, low_p_flag, lane_side); % calling steering function
...
function [throttle, velocity_err_hist] = longitudinal_controller(curr_state, velocity_err_hist, dt)
    ...
    MIN_HEAD_RATE = 0;
    MAX_HEAD_RATE = 0.3;
    curr_heading_rate = abs(curr_state(6));
    desired_vel = mapfun(curr_heading_rate, MIN_HEAD_RATE, MAX_HEAD_RATE, MIN_DES_VEL, MAX_DES_VEL);
...
function delta = stanleyController(lpinp, ref_track, curr_state, low_p_flag)
    ...
    curr_pos = [X + a * cos(psi); Y + a * sin(psi)];    % working with reference to the front axle of the vehicle
    [desired_heading, cross_track_error, ~, ~] = check_pos(lpinp, ref_track, curr_state, curr_pos, psi, low_p_flag);
    psi_e = desired_heading - psi;
    % control law
    des_steer = psi_e + atan2(ke * cross_track_error, (kv + u));
    des_steer = wrap(des_steer);
    des_steer = clamp(des_steer, -0.5, 0.5);
...
function [move, accumlated_poserr, lane_side, FLAG_terminate] = steering_obs(lpinp, ref_track, curr_state,lastoutput,outputsteps,accumlated_poserr,Xobs, avoid_obs_flag, low_p_flag, lane_side)
...
%     howfar = norm(refpt - curr_pt);
    [matchpt, howfar, whichside, FLAG_terminate] = check_pos(lpinp, new_ref_track, curr_state, curr_pt, curr_state(5), low_p_flag);
